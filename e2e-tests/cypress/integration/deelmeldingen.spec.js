...
describe.skip('Deelmeldingen', () => {
  // Skipped because deelmeldingen doesn't work in current e2e setup
  describe('Create Deelmeldingen', () => {
    ...
    describe('Create Deelmeldingen', () => {
      ...
      it('Should create 3 deelmeldingen from signal', () => {
        ...
        cy.route('PATCH', `/signals/v1/private/signals/${Cypress.env('signalId')}`).as('patchSignal');
        cy.route(`/signals/v1/private/signals/${Cypress.env('signalId')}/children/`).as('getDeelmeldingen');
        cy.get('[href*="/manage/incident/"]').contains(Cypress.env('signalId')).click();
        cy.get(SIGNAL_DETAILS.buttonCreateDeelmelding).click();
        cy.url().should('include', `/manage/incident/${Cypress.env('signalId')}/split`);
