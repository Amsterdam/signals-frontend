...
describe('Create signal "Bedrijven overlast stank" and check signal details', () => {
  describe('Create signal overlast stank', () => {
    ...
    it('Should create the signal', () => {
      ...
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_wat.label
      ).should('not.be.visible')
      cy.get(BEDRIJVEN_HORECA.radioButtonAnderBedrijf).click({ force: true })
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_naam.label
      ).should('not.be.visible')
      cy.get(BEDRIJVEN_HORECA.inputWieWat).type('Klanten van de sportsschool')
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_adres.label
      ).should('not.be.visible')
      cy.get(BEDRIJVEN_HORECA.inputAdres).type('Op de Klapperweg nummer 39')
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_stank.label
      ).should('be.visible')
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_stank
          .subtitle
      ).should('be.visible')
      cy.get(STANK_OVERLAST.inputGeur).type('Een zeer indringende zweetgeur')
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_stank_oorzaak
          .label
      ).should('be.visible')
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_stank_oorzaak
      ...
      cy.get(STANK_OVERLAST.inputOorzaakGeur).type(
        'Klanten van de sportschool die voor de deur staan te sporten, maar ook binnen voor een open raam'
      )
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_stank_weer
          .label
      ).should('be.visible')
      cy.get(STANK_OVERLAST.inputWeersomstandigheden).type(
        'Het is erg warm buiten, de zon schijnt volop'
      )
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_stank_ramen
          .label
      ).should('be.visible')
      cy.get(STANK_OVERLAST.radioButtonRaamOpen).click({ force: true })
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_vaker.label
      ).should('be.visible')
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_vaker
          .subtitle
      ).should('be.visible')
      cy.get(BEDRIJVEN_HORECA.radioButtonVakerNee).click({ force: true })
      cy.contains(
        questions.overlastBedrijvenEnHoreca.extra_bedrijven_horeca_caution
          .answers
      ).should('be.visible')
