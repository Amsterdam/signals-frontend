...
describe('Manage categories', () => {
  describe('Change category ', () => {
    ...
    it('Should change the attributes of the category and shows changes in history', () => {
      general.openMenu()
      cy.contains('IInstellingen').click()
      cy.contains('SSubcategorieën').click()
      ...
      cy.get(CATEGORIES.verantwoordelijkeAfdeling)
        .should('have.text', 'VOR, STW')
        .and('be.visible')
      ...
      cy.get(CATEGORIES.inputMessage)
        .clear()
        .type(
          'Ik beoordeel deze melding niet, het lijkt me namelijk allemaal onzin'
        )
      cy.get(CATEGORIES.inputNote)
        .clear()
        .type('Naamloze notities noteren, oftwel nnn.')
      ...
      cy.get(CATEGORIES.notification).should(
        'contain',
        'Subcategorie bijgewerkt'
      )
      ...
      general.checkHeaderText('Subcategorieën')
      cy.get(CATEGORIES.categoryValue)
        .eq(0)
        .should('contain', 'Afgewaterde brug')
      cy.get(CATEGORIES.categoryValue).eq(1).should('contain', '4 dagen')

      cy.contains('Afgewaterde brug').click()
      cy.url().should('include', 'instellingen/categorie/')
      cy.get(CATEGORIES.inputNote)
        .should('have.text', 'Naamloze notities noteren, oftwel nnn.')
        .and('be.visible')
      cy.get(CATEGORIES.historyAction)
        .eq(0)
        .should('contain', 'Afhandeltermijn gewijzigd naar')
        .and('contain', '4 weekdagen')
      cy.get(CATEGORIES.historyAction)
        .eq(0)
        .should('contain', 'Naam gewijzigd naar:')
        .and('contain', 'Afgewaterde brug')
      cy.get(CATEGORIES.historyAction)
        .eq(0)
        .should('contain', 'Servicebelofte gewijzigd naar:')
        .and(
          'contain',
          'Ik beoordeel deze melding niet, het lijkt me namelijk allemaal onzin'
        )
      cy.get(CATEGORIES.historyAction)
        .eq(0)
        .should('contain', 'Omschrijving gewijzigd naar:')
        .and('contain', 'Dit is het verhaal van de brug die moest afwateren')
    })
    it('Should change the status of a category to inactive', () => {
      ...
      // Change category
      cy.get(CATEGORIES.radioButtonNietActief)
        .click({ force: true })
        .should('be.checked')
      cy.get(CATEGORIES.buttonOpslaan).click()
      ...
      cy.get(CATEGORIES.historyAction)
        .eq(0)
        .should('contain', 'Status gewijzigd naar:')
        .and('contain', 'Inactief')
  ...
  describe('Create a signal and validate changes of category', () => {
    ...
    it('Should show the change in category description', () => {
      ...
      cy.get(CHANGE_CATEGORY.inputCategory)
        .find(':selected')
        .should('contain', 'Afgewaterde brug')
      cy.get(SIGNAL_DETAILS.infoText).should(
        'contain',
  ...
  describe('Change back servicebelofte', () => {
    ...
    it('Change back servicebelofte of category', () => {
      ...
      cy.visit('/instellingen/categorieen/page/1')
      general.checkHeaderText('Subcategorieën')
      cy.get('[data-testid=dataViewBody] > [data-testid=dataViewBodyRow]', {
        timeout: 10000,
      })
        .first()
