...
describe('Manage departments', () => {
  ...
  describe('Visit department page', () => {
    ...
    it('Should visit the manage department page by menu', () => {
      ...
      cy.contains('AAfdelingen').click()

      routes.waitForCategoriesRoutes()
      cy.url().should('include', '/IIIinstellingen/afdelingen')
  ...
  describe('Edit department', () => {
    ...
    it('Should edit a department', () => {
      ...
      cy.get(DEPARTMENTS.departmentDetail)
        .find('h2')
        .should('have.text', 'Afdeling')
        .and('be.visible')
      cy.get(DEPARTMENTS.categoryLists)
        .contains('Verantwoordelijk voor categorie')
        .and(($labels) => {
          expect($labels).to.have.css('color', 'rgb(236, 0, 0)')
        })
      cy.get(DEPARTMENTS.categoryLists)
        .contains('Toegang tot categorie')
        .and(($labels) => {
          expect($labels).to.have.css('color', 'rgb(236, 0, 0)')
        })
      ...
      cy.get(DEPARTMENTS.checkboxAsbestAccu)
        .eq(1)
        .should('be.checked')
        .and('be.disabled')
      ...
      cy.get(DEPARTMENTS.notification).should(
        'have.text',
        "Afdeling 'Actie Service Centrum' bijgewerkt"
      )
    ...
    it('Should cancel editing a department', () => {
      cy.get('td')
        .eq(3)
        .should('not.contain', 'Wildplassen / poepen / overgeven')
      cy.get('tr').eq(2).click()
      cy.wait('@getDepartment')
      cy.get(DEPARTMENTS.checkboxWildplassenPoepen)
        .check({ force: true })
        .should('be.checked')
  ...
  describe('Edit responsible department and check signal category', () => {
    ...
    describe('Change signal category', () => {
      ...
      it('Should change signal category', () => {
        ...
        cy.get(CHANGE_CATEGORY.inputCategory).select(
          'Drank- / drugsoverlast (ASC, THO)'
        )
    ...
    describe('Change responsible department', () => {
      ...
      it('Should change the responsible department', () => {
        ...
        cy.wait('@getDepartment')
        cy.get(DEPARTMENTS.checkboxDrankDrugsOverlast)
          .check({ force: true })
          .should('be.checked')
        cy.get(DEPARTMENTS.buttonOpslaan).click()
      })
    ...
    describe('Should change signal again and check responsible department', () => {
      ...
      it('Should change signal category and check responsible department', () => {
        ...
        cy.get(SIGNAL_DETAILS.subCategory)
          .should('have.text', 'Drank- / drugsoverlast (CCA, ASC, THO)')
          .and('be.visible')
      })
    })
