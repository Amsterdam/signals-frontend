...
Cypress.Commands.add('checkAllDetails', fixturePath => {
  ...
  cy.fixture(fixturePath).then(json => {
    ...
    cy.checkDepartments(fixturePath);
...
// Cypress.Commands.add('checkDepartments', fixturePath => {
//   cy.fixture(fixturePath).then(json => {
//     // eslint-disable-next-line no-unused-vars
//     Object.entries(json.directing_departments).forEach(([, valueA]) => {
...
Cypress.Commands.add('checkDepartments', fixturePath => {
  cy.fixture(fixturePath).then(json => {
    // eslint-disable-next-line no-unused-vars
    Object.values(json.directing_departments).forEach((elementValue: any) => {
...
export const checkQuestions = fixturePath => {
  cy.fixture(fixturePath).then(json => {
    Object.values(json.extra_properties).forEach((elementAValue: any) => {
      ...
      if (elementAValue.answer.label) {
        ...
        Object.values(elementAValue.answer).forEach((elementBValue: any) => {
...
export const uploadFile = (fixturePath, itemNumber) => {
  cy.fixture(fixturePath).then(json => {
    cy.get(CREATE_SIGNAL.buttonUploadFile).then((subject: any) => {
      // eslint-disable-next-line promise/no-nesting
      cy.fixture(json.fixtures.attachments[itemNumber], 'base64').then((file: any) => {
