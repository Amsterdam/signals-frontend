...
node {
    ...
    stage("Build image") {
        tryStep "build", {
            def image = docker.build("build.app.amsterdam.nl:5000/ois/signals_frontend:${env.BUILD_NUMBER}")
            image.push()
        }
    }
}
    ...
    node {
        stage('Push acceptance image') {
            tryStep "image tagging", {
                def image = docker.image("build.app.amsterdam.nl:5000/ois/signals_frontend:${env.BUILD_NUMBER}")
                image.pull()
