...
Configuration for theming, map and API endpoint URLs is defined in `environment.conf.base.json`. Override the default configuration by creating the file `environment.conf.json`. This file is ignored by Git. Changes to the configuration file will only be picked up when the development server is restarted.
...
- `environment.conf.json` with specific configuration for theming, map and API endpoint URLs.
...
The logo displayed on the website is defined by the `logo.url` parameter in `environment.base.conf.json`. This can be a remote URL or a local file path. Define for instance `/logo.svg` and you can inject your own `logo.svg` when starting the docker container.
