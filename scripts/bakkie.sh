...
echo '<h1>signals links</h1>' > /var/www/html/signals/index.html
echo "<h2>Generated at `date`</h2>" >> /var/www/html/signals/index.html
find /var/www/html/signals -name index.html -printf "%T+\t%p\n" | sort -r | grep 'signals' | sed 's/^.*\/signals/\/signals/' | sed 's/\/index.html//' | sed 's/^.*$/<p><a href="\0">\0<\/a><p>/' >> /var/www/html/signals/index.html
echo Done
