...
OUTDIR=/var/www/html/signas/builds/$1
if [ ! -d ${OUTDIR} ];
	then mkdir -p ${OUTDIR};
fi
cp -r dist/* ${OUTDIR}
...
echo '<h1>signals links</h1>' > /var/www/html/signas/index.html
echo "<h2>Generated at `date`</h2>" >> /var/www/html/signals/index.html
find /var/www/html/signas -name index.html -printf "%T+\t%p\n" | sort -r | grep 'signas' | sed 's/^.*\/signas/\/signas/' | sed 's/\/index.html//' | sed 's/^.*$/<p><a href="\0">\0<\/a><p>/' >> /var/www/html/signas/index.html
echo Done
