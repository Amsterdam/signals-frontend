...
describe('DataView with and without headers, filter and/or data', () => {
  ...
  it('should render headers and filters before data', () => {
    const { getByTestId } = render(dataViewWithProps({ headers, filters, data }));
  ...
  it('should render without data and without headers or filters', () => {
    const { rerender, getByTestId } = render(dataViewWithProps({ headers, filters }));
  ...
  it('should render with only headers or only filters', () => {
    const { rerender, getByTestId } = render(dataViewWithProps({ headers, data }));
    ...
    expect(dataViewHeader.childNodes[0].dataset.testid).toBe('dataViewHeadersRow');

    rerender(dataViewWithProps({ filters, data }));

    expect(dataView.childNodes).toHaveLength(2);
    expect(dataViewHeader.childNodes).toHaveLength(1);
    expect(dataViewHeader.childNodes[0].dataset.testid).toBe('dataViewFiltersRow');
  });
});
...
describe('DataView with correct number of columns', () => {
  it('should render data view with correct number of columns', () => {
    ...
    const { rerender, getByTestId, queryAllByTestId } = render(dataViewWithProps({ headers: testHeaders }));
    ...
    expect(queryAllByTestId('dataViewHeadersRowHeading')).toHaveLength(MAX_NUMBER_OF_COLUMNS);
    ...
    expect(queryAllByTestId('dataViewFiltersRowHeading')).toHaveLength(MAX_NUMBER_OF_COLUMNS);
    ...
    expect(queryAllByTestId('dataViewBodyRowValue')).toHaveLength(MAX_NUMBER_OF_COLUMNS);
    ...
    expect(queryAllByTestId('dataViewFiltersRowHeading')).toHaveLength(testFilters.length);
    expect(filtersRow.lastChild).toHaveAttribute('colspan', String(MAX_NUMBER_OF_COLUMNS - testFilters.length));
    ...
    expect(queryAllByTestId('dataViewHeadersRowHeading')).toHaveLength(testHeaders.length);
    expect(headersRow.lastChild).toHaveAttribute('colspan', String(MAX_NUMBER_OF_COLUMNS - testHeaders.length));
    expect(filtersRow.childNodes).toHaveLength(MAX_NUMBER_OF_COLUMNS);
    expect(queryAllByTestId('dataViewFiltersRowHeading')).toHaveLength(MAX_NUMBER_OF_COLUMNS);
    ...
    expect(queryAllByTestId('dataViewBodyRowValue')).toHaveLength(Object.keys(dataValues).length);
    expect(dataRow.lastChild).toHaveAttribute('colspan', String(MAX_NUMBER_OF_COLUMNS - Object.keys(dataValues).length));
...
describe('DataView with data', () => {
  it('should render data correctly', () => {
    const { getByTestId, queryAllByTestId, getByText } = render(dataViewWithProps({ data }));
  ...
  it('should render correct number of columns and in correct column order', () => {
    ...
    const expectOrder = (order, negate = false) => (row, rowIDX) => row.childNodes
      .forEach(
        (column, idx) => {
          ...
          const check = data[rowIDX][order[idx]] === undefined ? '' : data[rowIDX][order[idx]];
    ...
    allDataRows.forEach(
      row => expect(row.childNodes).toHaveLength(headers.length)
    );
    ...
    const alphabeticReversedWithNonExistingColumns = [...alphabeticReversed, 'non_existing_1', 'non_existing_2'];

    rerender(dataViewWithProps({ data, columnOrder: alphabeticReversedWithNonExistingColumns }));

    expect(allDataRows).toHaveLength(data.length);
    allDataRows.forEach(
      row => expect(row.childNodes).toHaveLength(alphabeticReversedWithNonExistingColumns.length)
    );
    ...
    expect(allDataRows).toHaveLength(data.length);
    allDataRows.forEach(
      row => expect(row.childNodes).toHaveLength(allNonExistingColumns.length)
    );
  ...
  it('should not render invisible columns', () => {
    ...
    expect(allDataRows).toHaveLength(data.length);
    allDataRows.forEach(
      row => expect(row.childNodes).toHaveLength(headers.length - invisibleColumns.length)
    );
    allDataRows.forEach(
    ...
    expect(allDataRows).toHaveLength(data.length);
    allDataRows.forEach(
      row => expect(row.childNodes).toHaveLength(headers.length - invisibleColumns.length + 2)
    );
    allDataRows.forEach(
      (row, rowIDX) => invisibleColumns.forEach(
        invisibleColumn => {
  ...
  it('should render with onItemClick and primaryKeyColumn property', () => {
    ...
    rerender(dataViewWithProps({ data, onItemClick: onItemClickHandler, primaryKeyColumn: headers[1] }));

    fireEvent.click(allRows[0]);

    expect(onItemClickHandler).toHaveBeenCalledTimes(2);
    allRows.forEach(
      (row, rowIDX) => expect(row.dataset.itemId).toBe(String(data[rowIDX][headers[1]]))
    );

    rerender(dataViewWithProps({ data, onItemClick: onItemClickHandler, primaryKeyColumn: headers[0] }));

    fireEvent.click(allRows[0]);

    expect(onItemClickHandler).toHaveBeenCalledTimes(3);
    allRows.forEach(
      (row, rowIDX) => expect(row.dataset.itemId).toBe(String(data[rowIDX][headers[0]]))
    );
  });
});
