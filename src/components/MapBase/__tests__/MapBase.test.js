...
describe('components/Map', () => {
  it('should render the map', () => {
    const { getByTestId, queryByText } = render(<MapBase mapOptions={MAP_OPTIONS} />);
  ...
  it('should render the zoom control', () => {
    const { container, rerender } = render(<MapBase mapOptions={MAP_OPTIONS} />);

    expect(container.querySelector('button[title="Inzoomen"]')).not.toBeInTheDocument();

    rerender(<MapBase mapOptions={MAP_OPTIONS} hasZoomControls />);
