...
describe('components/SiteHeader', () => {
  ...
  it('should render a title', () => {
    ...
    rerender(withAppContext(<SiteHeader permissions={[]} location={{ pathname: '/' }} isAuthenticated />));

    // do show title in front office when authenticated
    expect(queryByText(title)).not.toBeNull();

    rerender(withAppContext(<SiteHeader permissions={[]} location={{ pathname: '/manage' }} />));

    // don't show title in back office when not authenticated
    expect(queryByText(title)).toBeNull();

    rerender(withAppContext(<SiteHeader permissions={[]} location={{ pathname: '/manage' }} isAuthenticated />));
  ...
  it('should render correctly when logged in', () => {
    const { container, queryByText } = render(
      withAppContext(<SiteHeader isAuthenticated permissions={[]} location={{ pathname: '/' }} />),
    );
