...
describe('containers/PageHeader', () => {
  ...
  it('should provide the PageHeader component with a title', () => {
    const { container, rerender } = render(
      withAppContext(<PageHeaderContainerComponent filter={filter} incidentsCount={null} />),
    );

    expect(container.firstChild.querySelector('h1').textContent).toEqual('Meldingen');

    rerender(withAppContext(<PageHeaderContainerComponent filter={filter} incidentsCount={10} />));

    expect(container.firstChild.querySelector('h1').textContent).toEqual('Meldingen (10)');

    rerender(withAppContext(<PageHeaderContainerComponent filter={{ name: 'Foo bar !!1!' }} incidentsCount={null} />));

    expect(container.firstChild.querySelector('h1').textContent).toEqual('Foo bar !!1!');

    rerender(withAppContext(<PageHeaderContainerComponent filter={{ name: 'Foo bar !!1!' }} incidentsCount={99} />));
  ...
  it('should provide the PageHeader component with a subtitle', () => {
    ...
    const { container } = render(
      withAppContext(<PageHeaderContainerComponent filter={filter} incidentsCount={null} query={query} />),
    );
