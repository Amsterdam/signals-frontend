...
describe('signals/incident-management/reducer', () => {
  ...
  it('should not change loading flag when something is still loading', () => {
    ...
    const applied = (state) =>
      state
        ...
        .set('districts', fromJS(districts))
    ...
    expect(
      reducer(initialStateWithloadingFilters, getDistrictsSuccess)
    ).toEqual(applied(initialStateWithloadingFilters))
    expect(
      reducer(intermediateStateWithloadingFilters, getDistrictsSuccess)
    ).toEqual(applied(intermediateStateWithloadingFilters))
  })
  ...
  it('should handle GET_DISTRICTS_SUCCESS', () => {
    ...
    const applied = (state) =>
      state.set('loading', false).set('districts', fromJS(districts))

    expect(reducer(initialState, getDistrictsSuccess)).toEqual(
      applied(initialState)
    )
    expect(reducer(intermediateState, getDistrictsSuccess)).toEqual(
  ...
  it('should handle GET_DISTRICTS_FAILED', () => {
    ...
    expect(reducer(initialState, getDistrictsFailed)).toEqual(
      applied(initialState)
    )
    expect(reducer(intermediateState, getDistrictsFailed)).toEqual(
      applied(intermediateState)
    )
  })
  ...
  it('should handle FILTER_EDIT_CANCELED', () => {
    ...
    const applied = (state) =>
      state.set('editFilter', state.get('activeFilter'))
  ...
  it('should handle PAGE_CHANGED', () => {
    ...
    const applied = (state) =>
      state.set('page', page).set('loadingIncidents', true)
