...
describe('signals/incident-management/selectors', () => {
  ...
  it('should select fixtures', () => {
    const result = makeSelectFixtures.resultFunc(
      area,
      source,
      maincategory_slug,
      category_slug,
  ...
  it('should select all filters', () => {
    ...
    expect(allFilters[0].options.maincategory_slug[0].slug).toEqual(
      filters[0].options.maincategory_slug[0]
    )
    expect(allFilters[6].options.directing_department[0].key).toEqual(
      directing_department[1].key
    )
  })
  ...
  describe('active filter', () => {
    ...
    it('should return an empty object when the backend data is not present', () => {
      ...
      expect(
        makeSelectActiveFilter.resultFunc(initialState, { area, source })
      ).toEqual({})
    })
    ...
    it('should select active directing department filter', () => {
      const state = fromJS({
        ...initialState.toJS(),
        activeFilter: directingDepartmentFilter,
      })
      const actual = makeSelectActiveFilter.resultFunc(state, {
        area,
        source,
    ...
    it('should select active priority filter', () => {
      ...
      expect(actual.options.priority[0].key).toEqual(
        priorityFilter.options.priority[0]
      )
    })
  })
  ...
  describe('edit filter', () => {
    ...
    it('should select directing department filter', () => {
      const state = fromJS({
        ...initialState.toJS(),
        editFilter: directingDepartmentFilter,
      })
      const actual = makeSelectEditFilter.resultFunc(state, {
        area,
        source,
