...
describe('<Attachments />', () => {
  ...
  describe('rendering', () => {
    ...
    it('should render empty list when no attachments are defined', () => {
      const { queryByTestId, queryAllByTestId } = render(renderWithContext({ attachments: [] }));
  ...
  describe('events', () => {
    it('should trigger opening the attachment', () => {
      ...
      expect(preview).toHaveBeenCalledWith('attachment', { attachmentHref: attachmentProps.attachments[0].location });
    });
  });
});
