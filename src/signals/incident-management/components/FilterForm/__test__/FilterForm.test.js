...
describe('signals/incident-management/components/FilterForm', () => {
  ...
  it('should render filter fields', () => {
    const { container } = render(withAppContext(<FilterForm categories={categories} onSubmit={() => {}} />));
  ...
  it('should render groups of category checkboxes', () => {
    ...
    // category groups
    expect(container.querySelectorAll('input[type="checkbox"][name="maincategory_slug"]')).toHaveLength(
      Object.keys(categories.mainToSub).length,
    );

    Object.keys(categories.mainToSub).forEach(category => {
      expect(container.querySelectorAll(`input[type="checkbox"][name="${category}_category_slug"]`)).toHaveLength(
        Object.keys(categories.mainToSub[category]).length,
      );
    });
  });
  ...
  it('should render a list of priority options', () => {
    const { container, rerender, queryByTestId } = render(
      withAppContext(<FilterForm categories={categories} priorityList={null} onSubmit={() => {}} />),
    );

    expect(queryByTestId('priorityFilterGroup')).toBeNull();

    expect(container.querySelectorAll('input[type="radio"][name="priority"]')).toHaveLength(0);

    cleanup();

    rerender(withAppContext(<FilterForm categories={categories} priorityList={[]} onSubmit={() => {}} />));
    ...
    rerender(withAppContext(<FilterForm categories={categories} priorityList={priorityList} onSubmit={() => {}} />));

    expect(container.querySelectorAll('input[type="radio"][name="priority"]')).toHaveLength(priorityList.length + 1);
  });
  ...
  it('should render a list of status options', () => {
    ...
    expect(container.querySelectorAll('input[type="checkbox"][name="status"]')).toHaveLength(0);

    cleanup();

    rerender(withAppContext(<FilterForm categories={categories} statusList={[]} onSubmit={() => {}} />));
    ...
    rerender(withAppContext(<FilterForm categories={categories} statusList={statusList} onSubmit={() => {}} />));
  ...
  it('should render a list of stadsdeel options', () => {
    ...
    rerender(withAppContext(<FilterForm categories={categories} stadsdeelList={[]} onSubmit={() => {}} />));
    ...
    rerender(withAppContext(<FilterForm categories={categories} stadsdeelList={stadsdeelList} onSubmit={() => {}} />));
  ...
  it('should render a list of feedback options', () => {
    ...
    expect(container.querySelectorAll('input[type="radio"][name="feedback"]')).toHaveLength(0);

    cleanup();

    rerender(withAppContext(<FilterForm categories={categories} onSubmit={() => {}} feedbackList={feedback} />));

    expect(container.querySelectorAll('input[type="radio"][name="feedback"]')).toHaveLength(feedback.length + 1); // by default, a radio button with an empty value is rendered
  });

  it('should render a datepicker', () => {
    const { container, rerender } = render(withAppContext(<FilterForm categories={categories} onSubmit={() => {}} />));

    expect(container.querySelectorAll('input[type="hidden"][name="incident_date"]')).toHaveLength(0);
    ...
    expect(container.querySelectorAll('input[type="hidden"][name="incident_date"]')).toHaveLength(1);
  });
  ...
  it('should handle reset', async () => {
    ...
    const { container } = render(
      withAppContext(<FilterForm categories={categories} onClearFilter={onClearFilter} onSubmit={() => {}} />),
    );
  ...
  it('should handle cancel', () => {
    ...
    const { getByTestId } = render(
      withAppContext(<FilterForm categories={categories} onCancel={onCancel} onSubmit={() => {}} />),
    );
  ...
  it('should watch for changes in name field value', () => {
    const { container } = render(
      withAppContext(
        <FilterForm categories={categories} activeFilter={{ name: 'My saved filter' }} onSubmit={() => {}} />,
      ),
    );
