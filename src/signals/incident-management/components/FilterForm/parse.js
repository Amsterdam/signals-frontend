...
const arrayFields = ['stadsdeel', 'maincategory_slug', 'status', 'category_slug'];
...
export const parseInputFormData = (filterData, dataLists) => {
  ...
  if (Object.keys(filterData).length) {
    ...
    Object.keys(filterData).forEach(fieldName => {
      if (arrayFields.includes(fieldName) && !Array.isArray(filterData[fieldName])) {
    ...
    Object.keys(parsed)
      ...
      .forEach(fieldName => {
        parsed[fieldName] = parsed[fieldName].map(value =>
          dataLists[fieldName].find(({ key, slug }) => key === value || slug === value),
        );
      });
  }
