...
describe('signals/incident-management/components/RadioButtonList', () => {
  it('should render a title', () => {
    ...
    rerender(withAppContext(<RadioButtonList options={priorityList} groupName="priority" title="Here be dragons" onChange={() => { }} />));
  ...
  it('should render empty selection button', () => {
    const { rerender } = render(
      withAppContext(<RadioButtonList options={priorityList} groupName="priority" onChange={() => {}} />)
    );

    expect(screen.getAllByRole('radio')).toHaveLength(priorityList.length + 1);

    rerender(
      withAppContext(<RadioButtonList options={priorityList} groupName="priority" hasEmptySelectionButton={false} onChange={() => {}} />)
    );
