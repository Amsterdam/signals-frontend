...
export const SelectInput = props => {
  ...
  const options = values.map(({ key, value, slug }) => (
    <option key={useSlug ? slug : key} title={key ? value : emptyOptionText || value} value={useSlug ? slug : key}>
    ...
    <div className="select-input">
      <div className="mode_input text rij_verplicht">
        ...
        <div className="select-input__control invoer">
          <select name="" id={`form${name}`} {...handler()} multiple={multiple} size={multiple ? listSize : ''}>
            {options}
          </select>
        </div>
      </div>
