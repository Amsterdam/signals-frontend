...
describe('signals/incident-management/components/CheckboxList', () => {
  ...
  it('should render a title ', () => {
    ...
    const { getByLabelText } = render(
      withAppContext(<CheckboxList groupName="newGroup" groupId="newGroup" options={statuses} title={title} />),
    );
  ...
  it('should render a toggle', () => {
    const { getAllByLabelText, rerender } = render(
      withAppContext(<CheckboxList groupName="newGroup" groupId="newGroup" options={statuses} />),
    );

    // should not render a toggle checkbox, expecting number of checkboxes to be equal to the number of options
    expect(document.querySelectorAll('input[type="checkbox"]')).toHaveLength(statuses.length);

    // still shouldn't render a toggle checkbox
    rerender(
      withAppContext(
        <CheckboxList groupName="newGroup" groupId="newGroup" options={statuses} toggleLabel="Toggle me" />,
      ),
    );
    ...
    expect(document.querySelector(`input[name="${toggleFieldName}"]`)).toBeTruthy();
  });
  ...
  it('should render a list of checkboxes ', () => {
    ...
    const { rerender } = render(
      withAppContext(<CheckboxList groupName={groupName} groupId={groupName} options={truncated} />),
    );
    ...
    rerender(
      withAppContext(
        <CheckboxList groupName="newGroup" groupId="newGroup" options={truncated} clusterName={clusterName} />,
      ),
    );

    const boxesWithClusterName = document.querySelectorAll(`input[type="checkbox"][name$="${clusterName}"]`);
  ...
  it('should set default checked', () => {
    ...
    rerender(
      withAppContext(
        <CheckboxList groupName="newGroup" groupId="newGroup" options={statuses} defaultValue={defaultValue} />,
      ),
    );

    const checkedBoxes = Array.from(container.querySelectorAll('input[type="checkbox"]')).filter(el => el.checked);
  ...
  it('should handle toggle interaction', () => {
    ...
    const toggleBox = container.querySelector(`input[type="checkbox"][name="${toggleFieldName}"]`);
    const individualBoxes = container.querySelectorAll(`input[type="checkbox"]:not([name="${toggleFieldName}"])`);
  ...
  it('should handle individual checkbox interaction without a toggle checkbox', () => {
    ...
    const individualBoxes = container.querySelectorAll('input[type="checkbox"]');

    const singleBox = individualBoxes.item(Math.floor(Math.random() * statuses.length));
