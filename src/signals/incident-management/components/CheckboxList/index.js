...
const CheckboxList = ({
  ...
  const setGroupChecked = (shouldBeChecked = true) => {
    const checkboxes = groupContainer.current.querySelectorAll('input[type="checkbox"]');
  ...
  const handleIndividualCheck = () => {
    const mainSlugCheckbox = groupContainer.current.querySelector(`input[type="checkbox"][name="${toggleFieldName}"]`);
