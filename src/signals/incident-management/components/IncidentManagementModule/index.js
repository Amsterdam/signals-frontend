...
export const IncidentManagementModuleComponent = ({ isAuthenticated, match: { url } }) =>
  !isAuthenticated ? (
    <Route component={LoginPage} />
  ) : (
    <Fragment>
      <Route exact path={`${url}/incidents`} render={incidentOverviewPageWrapper(url)} />
      <Route exact path={`${url}/incident/:id`} render={incidentDetailWrapper(url)} />
      <Route exact path={`${url}/incident/:id/split`} render={incidentSplitContainerWrapper(url)} />
