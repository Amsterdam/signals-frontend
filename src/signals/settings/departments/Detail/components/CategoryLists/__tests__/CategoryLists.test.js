...
const findByMain = parentKey => subCategories.filter(category => category.parentKey === parentKey);
...
const checkedIsResponsible = department.categories.filter(({ is_responsible }) => is_responsible).length;
const checkedCanView = department.categories.filter(({ is_responsible, can_view }) => can_view && !is_responsible)
  .length;
const totalChecked = checkedIsResponsible * 2 + checkedCanView;

const withContextProvider = Component =>
  withAppContext(
    <DepartmentDetailContext.Provider value={{ categories, subCategories, department, findByMain }}>
      {Component}
...
describe('signals/settings/departments/Detail/components/CategoryLists', () => {
  ...
  it('should render groups of checkboxes', () => {
    const { getByText } = render(withContextProvider(<CategoryLists {...props} />));

    expect(document.querySelectorAll('input[type=checkbox]')).toHaveLength(subCategories.length * 2);
  ...
  it('should render a form', () => {
    const { container } = render(withContextProvider(<CategoryLists {...props} />));
  ...
  it('should update the state on is_responsible checkbox tick', () => {
    ...
    const { container, getByText } = render(withContextProvider(<CategoryLists {...props} />));

    const checkbox = getByText('Verantwoordelijk voor categorie')
      .closest('fieldset')
      .querySelector('input[type="checkbox"]:not(:checked)');

    expect(container.querySelectorAll('input:checked')).toHaveLength(totalChecked);
    ...
    expect(container.querySelectorAll('input:checked')).toHaveLength(totalChecked + 2);
  });
  ...
  it('should update the state on is_responsible toggle click', () => {
    ...
    const isResponsibleFieldset = getByText('Verantwoordelijk voor categorie').closest('fieldset');

    const toggleIsResponsible = getAllByText(isResponsibleFieldset, 'Alles selecteren')[0];
    const checkboxListIsResponsible = toggleIsResponsible.closest('[data-testid="checkboxList"]');
    const numBoxes = checkboxListIsResponsible.querySelectorAll('input').length;
    const numTickedIsResponsible = checkboxListIsResponsible.querySelectorAll(':checked').length;

    const canViewFieldset = getByText('Toegang tot categorie').closest('fieldset');

    const checkboxListCanView = getAllByText(canViewFieldset, 'Alles selecteren')[0].closest(
      '[data-testid="checkboxList"]'
    );

    const numTickedCanView = checkboxListCanView.querySelectorAll(':checked').length;
    ...
    const extraTicked = numBoxes - numTickedIsResponsible + (numBoxes - numTickedCanView);

    expect(container.querySelectorAll('input:checked')).toHaveLength(totalChecked + extraTicked);
  ...
  it('should update the state on can_view checkbox tick', () => {
    ...
    expect(container.querySelectorAll('input:checked')).toHaveLength(totalChecked + 1);
  });
  ...
  it('should update the state on can_view toggle click', () => {
    ...
    const checkboxList = getAllByText(canViewFieldset, 'Alles selecteren')[0].closest('[data-testid="checkboxList"]');
    ...
    expect(container.querySelectorAll('input:checked')).toHaveLength(totalChecked - numTickedNotEnabled);
  });
});
