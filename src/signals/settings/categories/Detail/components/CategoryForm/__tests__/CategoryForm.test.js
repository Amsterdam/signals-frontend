...
describe('signals/settings/categories/Detail/components/CategoryForm', () => {
  it('should render the correct fields', () => {
    ...
    expect(
      container.querySelector('[name="use_calendar_days"]')
    ).toBeInTheDocument();
    expect(container.querySelector('[name="use_calendar_days"]').value).toBe(
      '0'
    );
  ...
  it('should set field values', () => {
    ...
    const { container, rerender } = render(
      withAppContext(<CategoryForm data={data} />)
    );
    ...
    expect(container.querySelector('[name="handling_message"]').value).toBe(
      data.handling_message
    );
    expect(container.querySelector('[name="n_days"]').value).toBe(
      `${data.sla.n_days}`
    );
    ...
    rerender(
      withAppContext(
        <CategoryForm
          data={{ ...data, sla: { n_days: 10, use_calendar_days: true } }}
        />
      )
    );
  ...
  it('should call onCancel callback', () => {
    ...
    const { getByTestId } = render(
      withAppContext(<CategoryForm onCancel={onCancel} />)
    );
  ...
  it('should call onSubmit callback', () => {
    ...
    const tree = mount(
      withAppContext(<CategoryForm onSubmitForm={onSubmit} />)
    );
