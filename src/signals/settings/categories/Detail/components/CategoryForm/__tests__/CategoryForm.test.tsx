...
describe('signals/settings/categories/Detail/components/CategoryForm', () => {
  const defaultProps = {
    ...
    history: [],
    onCancel: jest.fn(),
    onSubmitForm: jest.fn((event: React.MouseEvent) => { event.preventDefault(); }),
    readOnly: false,
  };
  ...
  it('should render the correct fields', () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Naam' })).toHaveValue('Mock name');
    expect(screen.getByRole('textbox', { name: 'Omschrijving' })).toHaveValue('Mock description');
  ...
  it('should render category history', () => {
    const { rerender } = render(withAppContext(<CategoryForm {...defaultProps} />));
    ...
    rerender(withAppContext(<CategoryForm {...defaultProps} history={history} />));
  ...
  it('should make fields disabled', () => {
    ...
    screen.getAllByRole('textbox').forEach(element => { expect(element).not.toBeDisabled(); });
    screen.getAllByRole('spinbutton').forEach(element => { expect(element).not.toBeDisabled(); });
    screen.getAllByRole('combobox').forEach(element => { expect(element).not.toBeDisabled(); });
    screen.getAllByRole('radio').forEach(element => { expect(element).not.toBeDisabled(); });
    ...
    screen.getAllByRole('spinbutton').forEach(element => { expect(element).toBeDisabled(); });
    screen.getAllByRole('combobox').forEach(element => { expect(element).toBeDisabled(); });
    screen.getAllByRole('radio').forEach(element => { expect(element).toBeDisabled(); });
  ...
  it('should set field values', () => {
    ...
    const { rerender, unmount } = render(withAppContext(<CategoryForm {...props} />));
    ...
    expect(screen.getByRole('textbox', { name: 'Omschrijving' })).toHaveValue('Bar');
    expect(screen.getByRole('textbox', { name: 'Servicebelofte' })).toHaveValue('foo@bar');
    ...
    rerender(withAppContext(<CategoryForm {...defaultProps} data={{ ...props.data, sla: { n_days: 10, use_calendar_days: true }, departments: [] } as CategoryFormProps['data']} />));
