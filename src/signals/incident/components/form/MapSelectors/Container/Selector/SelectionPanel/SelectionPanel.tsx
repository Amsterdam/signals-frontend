...
const SelectionPanel: FunctionComponent<SelectionPanelProps> = ({
  ...
  const selectionOnMap = useMemo(
    () => selection.filter((container) => container.type !== UNREGISTERED_TYPE),
    [selection]
  )
  const unregisteredContainer = useMemo(
    () => selection.find((container) => container.type === UNREGISTERED_TYPE),
    [selection]
  )
  const hasUnregisteredContainer = useMemo(
    () => Boolean(unregisteredContainer),
  ...
  const updateUnregisteredContainer = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      if (unregisteredContainer) {
        onChange([
          ...selectionOnMap,
          { ...unregisteredContainer, id: event.currentTarget.value },
        ])
      }
    },
  ...
  return (
    ...
    >
      ...
      {unregisteredFeature && (
        <div>
          <Checkbox
            id="unregisteredContainerCheckbox"
            checked={hasUnregisteredContainer}
            onChange={toggleUnregisteredContainer}
          />
          <Label
            htmlFor="unregisteredContainerCheckbox"
            label="De container staat niet op de kaart"
          />
          ...
          {unregisteredContainer && (
            <Fragment>
              <Label
                ...
                label={
                  <Fragment>
                    <strong>Wat is het nummer van de container?</strong> (niet
                    verplicht)
                  </Fragment>
                }
              />
              <Input
                id="unregisteredContainerInput"
                onSubmit={onClose}
                onKeyDown={handleKeyDown}
                onChange={updateUnregisteredContainer}
                value={unregisteredContainer.id}
              />
            </Fragment>
          )}
        </div>
