...
import { computeNrOfIncidentsPerFilter } from './compute-nr-of-incidents-per-filter'

describe('computeNrOfIncidentsPerFilter', () => {
  afterEach(() => {
    jest.resetAllMocks()
  })
  it('should add 1 to nrOfIncidents', () => {
    ...
    const result = computeNrOfIncidentsPerFilter(
      mockFiltersOneItem,
      mockIncidentsShort
    )

    expect(result[0].nrOfIncidents).toBe(1)
  })

  it("should add 1 to nrOfIncidents of mainCategoryFilter foreach of it's subCategories", () => {
    const result = computeNrOfIncidentsPerFilter(
      mockFiltersShort,
      mockIncidentsShort
    )

    expect(result[0].nrOfIncidents).toBe(3)
  })
})
