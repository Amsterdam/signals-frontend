...
const getProxyConfig = ({ url, method }) => {
  const exactUrlMatch = ({ request }) => url.startsWith(request.url) && method === request.method;

  const patternUrlMatch = ({ request }) =>
    request.url.includes('*') && new RegExp(request.url.replace('*', '(.+)')).test(url) && method === request.method;
...
const handleFetch = event => {
  ...
  if (proxyConfig) {
    ...
    event.waitUntil(
      Promise.resolve(event.respondWith(response.delay ? delayResponse(response.delay, proxyResponse) : proxyResponse))
    );
  }
};
